...
async def get_clients(
    ...
    # Execute query
    cursor = db.clients.find(query).sort(sort_criteria).skip(skip).limit(limit)
...
async def get_client_stats(current_user: UserModel = Depends(get_current_user), db: ScopedDatabase = Depends(get_db)):
    ...
    this_month = await db.clients.count_documents(month_query)
...
@router.get("/{id}")
async def get_client(id: str, current_user: UserModel = Depends(get_current_user), db: ScopedDatabase = Depends(get_db)):
...
async def create_client(client: ClientModel = Body(...), current_user: UserModel = Depends(get_current_user), db: ScopedDatabase = Depends(get_db)):
    """CREATE: Add a new client"""
    client.agency_id = current_user.agency_id # Helper, redundant with wrapper but good for model consistency
...
async def update_client(client_id: str, update_data: dict = Body(...), current_user: UserModel = Depends(get_current_user), db: ScopedDatabase = Depends(get_db)):
...
async def delete_client(client_id: str, current_user: UserModel = Depends(get_current_user), db: ScopedDatabase = Depends(get_db)):
    ...
    delete_result = await db.clients.delete_one({"_id": ObjectId(client_id)})
