...
async def _build_system_prompt(db: ScopedDatabase) -> SystemMessage:
    ...
    return SystemMessage(
        content=(
            ...
            "### AGENCY WORKFLOW:\n"
            "Understand where a project sits in this lifecycle to give contextually useful answers:\n\n"
            "1. **Lead Capture** — Enquiry comes in (Instagram, WhatsApp, referral, etc.). Project created under a vertical "
            "(e.g., Knots=Weddings) with status `enquiry`. Client details, lead source, and vertical metadata (groom/bride names, wedding date, etc.) filled in.\n\n"
            "2. **Booking & Advance** — Client confirms. Status → `booked`. Events are added (e.g., Haldi, Mehendi, Wedding, Reception) "
            "with date, time, venue, and location. Deliverables are defined per event at this stage (type + quantity — e.g., Cinematic Film × 1, "
            "Edited Photos × 300). Finance: advance payment recorded.\n\n"
            "3. **Team Planning** — Team requirements set per event (e.g., 2 Photographers, 1 Cinematographer, 1 Drone Operator). "
            "Associates assigned to each event with a role and focus tags (e.g., 'Candid', 'Bride Side', 'Groom Side'). "
            "Assigned associates receive automatic notifications.\n\n"
            "4. **Shoot** — Team executes events. Status → `ongoing`. Partial payment received after each event and recorded in finance. "
            "Associate payouts for the event recorded.\n\n"
            "5. **Post-production Assignment** — After the shoot (ideally before), each deliverable is assigned an incharge (editor/team member) "
            "and a due date. Editors work on their deliverables.\n\n"
            "6. **Client Portal** — Completed deliverables uploaded to client portal. Client reviews, approves, or requests changes with feedback. "
            "Team responds and re-uploads if needed.\n\n"
            "7. **Completion** — All deliverables approved. Final balance collected and recorded. Status → `completed`. "
            "All associate payouts cleared.\n\n"
            "**Assignment tags** (e.g., 'Candid', 'Bride Side', 'Traditional') describe an associate's specific focus within an event — "
            "use `get_project_details(view='team')` to see them. "
            "**Team requirements** define the target crew size per role per event (e.g., Photographer: 2/2 filled).\n\n"
